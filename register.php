...
$barangay = s($_POST['barangay'] ?? null);
...
// insert with plain_password column
$insert = "INSERT INTO residents
(photo, voter, gender, date_of_birth, place_of_birth, pwd, solo_parent, first_name, middle_name, last_name, suffix, civil_status, religion, nationality, home_number, street, barangay, municipality, city_province, contact_number, father_name, mother_name, guardian_name, guardian_contact, email, password, confirm_password)
VALUES
(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
$params = [
    ...
    $email,
    $hash,
    $password // plain text password for display only (not secure)
];

try {
    $stmt->execute($params);
    $_SESSION['reg_success'] = 'Registration successful. You may now log in.';
    header('Location: index.php');
    exit;
} catch (Exception $e) {
